...
const goals = {
  search:
    'Open the catalog search and type "iPhone 15 case", then stop when results are showing.',
  settings:
    "Open the Settings app, drill into the 'About phone' entry. Stop when the Android version is visible.",
};
const escape = (value) =>
...
function render() {
  ...
  const labels = {
    ...
    done: "Jev reports complete · inspect the screen",
  ...
  $("history").innerHTML = state.history.length
    ? state.history
        .map(
          (h) =>
            `<div class="trace-row"><span class="number">${String(h.step).padStart(2, "0")}</span><div>${escape(h.action)}${h.text ? ` <b>“${escape(h.text)}”</b><small>${escape(h.text_helper)}</small>` : ""}</div><span class="time">${h.latency_ms} ms · ${percent(h.probability)}</span><span class="effect">${h.page_changed ? "Screen changed" : "No change observed"}</span></div>`,
        )
        .join("")
...
$("download").addEventListener("click", () => {
  ...
  a.href = url;
  a.download = "my-autox-trace.json";
  a.click();
